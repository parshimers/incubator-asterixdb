...
public class DelimitedDataParser extends AbstractDataParser implements
	...
	protected IARecordBuilder recBuilder;
	...
	public boolean parse(DataOutput out) throws AsterixException, IOException {
		while (cursor.nextRecord()) {
			...
			for (int i = 0; i < valueParsers.length; ++i) {
				...
				if (cursor.fStart == cursor.fEnd
					...
					if (recordType.getFieldTypes()[i].getTypeTag() != ATypeTag.UNION
							|| !NonTaggedFormatUtil
									.isOptionalField((AUnionType) recordType
											.getFieldTypes()[i])) {
					...
					ANullSerializerDeserializer.INSTANCE.serialize(ANull.NULL,
							out);
				} else {
			...
			if (!areAllNullFields) {
				writeDataRecord(recBuilder, out, true);
				return true;
			}
		}
		return false;
	...
	protected void writeDataRecord(IARecordBuilder recBuilder, DataOutput out,
			boolean writeTypeTag) throws IOException, AsterixException {
		recBuilder.write(out, writeTypeTag);
	}
