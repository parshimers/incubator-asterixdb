...
public class ADMDataParser extends AbstractDataParser implements IDataParser {
	...
	public boolean parse(DataOutput out) throws AsterixException {
		try {
			...
			throw new ParseException(e, admLexer.getLine(),
					admLexer.getColumn());
	...
	public void initialize(InputStream in, ARecordType recordType,
			boolean datasetRec) throws AsterixException {
	...
	private void admFromLexerStream(int token, IAType objectType,
		...
		case AdmLexer.TOKEN_STRING_LITERAL: {
			if (checkType(ATypeTag.STRING, objectType)) {
				final String tokenImage = admLexer
						.getLastTokenImage()
						.substring(1, admLexer.getLastTokenImage().length() - 1);
				aString.setValue(admLexer.containsEscapes() ? replaceEscapes(tokenImage)
						: tokenImage);
		...
		case AdmLexer.TOKEN_INTERVAL_DATE_CONS: {
			if (checkType(ATypeTag.INTERVAL, objectType)) {
				if (admLexer.next() == AdmLexer.TOKEN_CONSTRUCTOR_OPEN) {
					if (admLexer.next() == AdmLexer.TOKEN_STRING_LITERAL) {
						AIntervalSerializerDeserializer.parseDate(
								admLexer.getLastTokenImage(), out);
			...
			throw new ParseException(
					"Wrong interval data parsing for date interval.");
		}
		case AdmLexer.TOKEN_INTERVAL_TIME_CONS: {
			if (checkType(ATypeTag.INTERVAL, objectType)) {
				if (admLexer.next() == AdmLexer.TOKEN_CONSTRUCTOR_OPEN) {
					if (admLexer.next() == AdmLexer.TOKEN_STRING_LITERAL) {
						AIntervalSerializerDeserializer.parseTime(
								admLexer.getLastTokenImage(), out);
			...
			throw new ParseException(
					"Wrong interval data parsing for time interval.");
		}
		case AdmLexer.TOKEN_INTERVAL_DATETIME_CONS: {
			if (checkType(ATypeTag.INTERVAL, objectType)) {
				if (admLexer.next() == AdmLexer.TOKEN_CONSTRUCTOR_OPEN) {
					if (admLexer.next() == AdmLexer.TOKEN_STRING_LITERAL) {
						AIntervalSerializerDeserializer.parseDatetime(
								admLexer.getLastTokenImage(), out);
			...
			throw new ParseException(
					"Wrong interval data parsing for datetime interval.");
		}
		...
		default: {
			throw new ParseException("Unexpected ADM token kind: "
		...
		while (idx < len) {
			if (chars[idx] == '\\') {
				...
				case '\"':
					...
					break;
				default:
					throw new ParseException("Illegal escape '\\"
							+ chars[idx + 1] + "'");
				}
			}
			++idx;
	...
	private ATypeTag getTargetTypeTag(ATypeTag expectedTypeTag,
			IAType aObjectType) throws IOException {
		if (aObjectType == null) {
			return expectedTypeTag;
		}
		if (aObjectType.getTypeTag() != ATypeTag.UNION) {
			final ATypeTag typeTag = aObjectType.getTypeTag();
			return ATypeHierarchy.canPromote(expectedTypeTag, typeTag) ? typeTag
					: null;
		} else { // union
			List<IAType> unionList = ((AUnionType) aObjectType).getUnionList();
	...
	private void parseRecord(ARecordType recType, DataOutput out,
			...
			case AdmLexer.TOKEN_STRING_LITERAL: {
				...
				if (recType != null) {
					...
					if (fieldId < 0 && !recType.isOpen()) {
						...
						aStringFieldName
								.setValue(admLexer.getLastTokenImage()
										.substring(
												1,
					...
					aStringFieldName.setValue(admLexer.getLastTokenImage()
							.substring(1,
									admLexer.getLastTokenImage().length() - 1));
					stringSerde.serialize(aStringFieldName,
							fieldNameBuffer.getDataOutput());
		...
		if (recType != null) {
			...
			if (nullableFieldId != -1)
				throw new ParseException("Field " + nullableFieldId
						+ " can not be null");
		}
		writeRecord(recBuilder, out, true);
	...
	protected void writeRecord(IARecordBuilder recordBuilder, DataOutput out,
			boolean writeTypeTag) throws IOException, AsterixException {
		...
		do {
			...
			if (token == AdmLexer.TOKEN_END_RECORD) {
				if (admLexer.next() == AdmLexer.TOKEN_END_RECORD) {
					...
					throw new ParseException(
							"Found END_RECORD while expecting a list item.");
				}
			} else if (token == AdmLexer.TOKEN_COMMA) {
				if (first) {
					throw new ParseException(
	...
	private void parseToNumericTarget(ATypeTag typeTag, IAType objectType,
		...
		if (targetTypeTag == null
			...
			throw new ParseException(mismatchErrorMessage
					+ objectType.getTypeName() + mismatchErrorMessage2
					+ typeTag);
		}
	}
	...
	private void parseAndCastNumeric(ATypeTag typeTag, IAType objectType,
		...
		if (targetTypeTag == null
				|| !parseValue(admLexer.getLastTokenImage(), typeTag,
						dataOutput)) {
			throw new ParseException(mismatchErrorMessage
					+ objectType.getTypeName() + mismatchErrorMessage2
					+ typeTag);
		...
		if (targetTypeTag != typeTag) {
			ITypePromoteComputer promoteComputer = ATypeHierarchy
					.getTypePromoteComputer(typeTag, targetTypeTag);
			// the availability if the promote computer should be consistent
			// with the availability of a target type
			assert promoteComputer != null;
			// do the promotion; note that the type tag field should be skipped
			promoteComputer.promote(castBuffer.getByteArray(),
	...
	private void parseConstructor(ATypeTag typeTag, IAType objectType,
			DataOutput out) throws AsterixException, AdmLexerException,
			IOException {
		final ATypeTag targetTypeTag = getTargetTypeTag(typeTag, objectType);
		if (targetTypeTag != null) {
			DataOutput dataOutput = out;
			...
			if (token == AdmLexer.TOKEN_CONSTRUCTOR_OPEN) {
				...
				if (token == AdmLexer.TOKEN_STRING_LITERAL) {
					final String unquoted = admLexer.getLastTokenImage()
							.substring(1,
									admLexer.getLastTokenImage().length() - 1);
					...
					if (token == AdmLexer.TOKEN_CONSTRUCTOR_CLOSE) {
						if (targetTypeTag != typeTag) {
							ITypePromoteComputer promoteComputer = ATypeHierarchy
									.getTypePromoteComputer(typeTag,
											targetTypeTag);
							// the availability if the promote computer should
							// be consistent with the availability of a target
							// type
							assert promoteComputer != null;
							// do the promotion; note that the type tag field
							// should be skipped
							promoteComputer.promote(castBuffer.getByteArray(),
									castBuffer.getStartOffset() + 1,
									castBuffer.getLength() - 1, out);
	...
	private boolean parseValue(final String unquoted, ATypeTag typeTag,
			DataOutput out) throws AsterixException, HyracksDataException,
			IOException {
		switch (typeTag) {
		case BOOLEAN:
		...
		case DAYTIMEDURATION:
			ADayTimeDurationSerializerDeserializer.INSTANCE
					.parse(unquoted, out);
			return true;
		case YEARMONTHDURATION:
			AYearMonthDurationSerializerDeserializer.INSTANCE.parse(unquoted,
					out);
			return true;
		case POINT:
	...
	private void parseInt8(String int8, DataOutput out)
			throws AsterixException, HyracksDataException {
