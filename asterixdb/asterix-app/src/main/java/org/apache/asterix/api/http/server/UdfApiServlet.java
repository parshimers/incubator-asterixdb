...
public class UdfApiServlet extends AbstractServlet {
    ...
    protected void post(IServletRequest request, IServletResponse response) {
        ...
        File udf = null;
        boolean binaryDistribution = false;
        try {
    ...
    private static void deleteLibrary(MetadataTransactionContext mdTxnCtx, DataverseName dataverse, String libraryName)
        ...
        for (Function function : functions) {
            if (libraryName.equals(function.getLibrary())) {
                throw new AsterixException(ErrorCode.METADATA_DROP_LIBRARY_IN_USE, dataverse.toString());
            }
        }
    ...
    private void deleteUdf(DataverseName dataverse, String resourceName) throws Exception {
        ...
        try {
            ...
            mdLockList = metadataProvider.getLocks();
            MetadataManager.INSTANCE.init();
            mdLockUtil.dropLibraryBegin(appCtx.getMetadataLockManager(), mdLockList, dataverse, resourceName);
            mdTxnCtx = MetadataManager.INSTANCE.beginTransaction();
