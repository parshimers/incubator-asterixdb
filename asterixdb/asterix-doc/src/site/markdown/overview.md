...
2. [AsterixDB 101: An ADM and SQL++ Primer](sqlpp/primer.html) :
...
The AsterixDB user model consists of its data modeling features (ADM) and its query capabilities (SQL++).
...
3. [Asterix Data Model (ADM)](sqlpp/datamodel.html), [Asterix Functions](sqlpp/functions.html), [Asterix functions for Allen's Relations](sqlpp/allens.html), and [Asterix Query Language (SQL++)](sqlpp/manual.html) :
These are reference documents that catalog the primitive data types and built-in functions available in SQL++ and the reference manual for SQL++ itself.
