...
public abstract class AbstractExperiment7Builder extends AbstractLSMBaseExperimentBuilder {
    ...
    protected abstract void doBuildPauseAction(SequentialActionList seq) throws Exception;

    @Override
    protected void doBuildDataGen(SequentialActionList seq, Map<String, List<String>> dgenPairs) throws Exception {
        //start datagen
        SequentialActionList pauseAction = new SequentialActionList();
        doBuildPauseAction(pauseAction);
        final OrchestratorServer oServer = new OrchestratorServer(orchPort, dgenPairs.size(), nIntervals, pauseAction);
